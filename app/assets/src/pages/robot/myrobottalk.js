...
class Container extends React.Component {
  ...
  autoLayOut(data) {
    ...
    for (let i = 0; i <= level; i++) {
      levelHeight[i] = Math.max(
        ...levelNodes[i].map(item => this.getEle(`topology-node-${item}`).offsetHeight)
      );
    ...
    for (let i = 0; i <= level; i++) {
      ...
      for (let j = 0; j < levelNodes[i].length; j++) {
        ...
        levelLeft =
          j == 0
            ? levelLeft
            : levelLeft + this.getEle(`topology-node-${levelNodes[i][j - 1]}`).offsetWidth + 25;
        let left = levelLeft;
        let top = levalTop;
  ...
  onDrop(id, left, top, type, name) {
    if (type === 'btn') {
      let newNode = { id, name, knowledge: '', action: {}, actionParam: {}, parentId: -1 };
      newNode.answerIds = [];
      if (name === '普通节点') {
            ...
            <div className="react-contextmenu-wrapper">
              <div className="topology-container">
                ...
                <div className="topology-tools">
                  ...
                  <div className="topology-tools-btn" onClick={this.autoLayOut.bind(this, data)}>
                    <Icon type="apartment" />
